...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── PLAN CHECK for all device operations (except deleteInstance, status, and report_wa devices) ──
    if (action !== "deleteInstance" && action !== "status" && action !== "getBaseUrl" && action !== "logout" && !isReportDevice) {
    ...
    // ── connect ──
    if (action === "connect") {
      // Auto-assign token from pool if device has no token (both normal and report_wa)
      if (!instanceToken) {
        console.log(`Device "${deviceName}" (${device?.login_type}) has no token — auto-assigning from pool...`);
