...
DECLARE
  ...
  _base_url TEXT;
BEGIN
  -- Only fire on status transitions to completed, paused, canceled, failed
  ...
  -- Fire-and-forget HTTP via pg_net
...
DROP TRIGGER IF EXISTS trg_campaign_wa_instant ON public.campaigns;
CREATE TRIGGER trg_campaign_wa_instant
  AFTER UPDATE ON public.campaigns
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_campaign_wa_instant();
