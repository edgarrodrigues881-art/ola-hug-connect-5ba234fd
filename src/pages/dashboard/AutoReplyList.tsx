            ...
            >
              ...
              <div className="flex items-start sm:items-center gap-4 px-5 py-4 pl-6">
                ...
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ring-1 transition-colors mt-0.5 sm:mt-0 ${
                  ...
                  <BotMessageSquare className={`w-4.5 h-4.5 ${model.isActive ? "text-emerald-500" : "text-muted-foreground/40"}`} />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                    <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/50">
                      ...
                      {triggerLabels[model.trigger] || model.trigger}
                    </span>
                    <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/40">
                      <GitBranch className="w-3 h-3" />
                      {model.steps} etapa{model.steps !== 1 ? "s" : ""}
                    </span>
                    <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/40">
                      <MousePointerClick className="w-3 h-3" />
                      {model.buttons} botão{model.buttons !== 1 ? "ões" : ""}
                    </span>
                    <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/40">
                      <Play className="w-3 h-3" />
                      {model.lastRun
                        ? format(model.lastRun, "dd MMM, HH:mm", { locale: ptBR })
                        : "Nunca executado"}
                    </span>
                    <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/30">
                      <Clock className="w-3 h-3" />
                      {format(model.updatedAt, "dd MMM, HH:mm", { locale: ptBR })}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-8 text-xs gap-1.5 border-border/40 hover:border-primary/40 hover:text-primary transition-colors"
