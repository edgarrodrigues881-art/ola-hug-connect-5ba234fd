...
const GradientCheck = ({ className = "" }: { className?: string }) => (
  <div className={cn("w-5 h-5 rounded-full flex items-center justify-center shrink-0 bg-primary/15", className)}>
    ...
    <div className="min-h-screen pb-20 bg-background">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-20">
        ...
        <div className="text-center max-w-2xl mx-auto pt-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold mb-6 border border-primary/15 bg-primary/10 text-primary">
            ...
            >
              ...
              {plan.popular && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-20">
                  <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.1em] px-4 py-1.5 rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/30">
              ...
              <div className="p-7 pt-9 flex flex-col h-full">
                ...
                <p className="text-[11px] mt-0.5 text-muted-foreground/35">
                  Cancele quando quiser
                </p>

                {/* Description */}
                <p className="text-[13px] leading-relaxed mt-4 mb-7 min-h-[40px] text-muted-foreground/55">
                  {plan.description}
                </p>
                ...
                <a
                  ...
                  className={cn(
                    "w-full py-3.5 font-semibold text-sm flex items-center justify-center gap-2 mb-7 transition-all duration-150 rounded-xl",
                    plan.popular
                      ...
                      : "bg-accent text-foreground border border-border hover:bg-accent/80 hover:border-border/80"
                  )}
                >
                  {plan.cta}
                ...
                {/* Divider */}
                <div className="h-px mb-6 bg-border/30" />
                  ...
                  {FEATURES.map((feature, i) => {
                    ...
                    return (
                      <div key={i} className={cn(
                        ...
                        included ? "text-foreground/70" : "text-muted-foreground/20"
                      )}>
                        {included ? (
                          <GradientCheck />
                        ) : (
                          <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 bg-muted/30">
                            <X className="w-3 h-3 text-muted-foreground/20" strokeWidth={2.5} />
                          </div>
                        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/30">
                  <th className="text-left px-5 py-4 text-xs font-semibold uppercase tracking-wider w-[200px] text-muted-foreground/50">Recurso</th>
                  {plans.map(p => (
                    <th key={p.name} className={cn(
                      "text-center px-3 py-4 text-xs font-bold uppercase tracking-wider",
              ...
              <tbody>
                {comparisonRows.map((row, ri) => (
                  <tr key={ri}
                    className={cn(
                      "transition-colors duration-150 border-t border-border/30 hover:bg-muted/20",
                      ri % 2 === 1 && "bg-muted/10"
                    )}
                  >
                      ...
                      <td key={vi} className="text-center px-3 py-3.5">
                        {typeof val === "boolean" ? (
                          val ? (
                            ...
                            <X className="w-4 h-4 mx-auto text-muted-foreground/15" strokeWidth={2.5} />
                          )
                        ) : (
                          <span className={cn(
                            "text-sm font-bold",
                            plans[vi].popular ? "text-primary" : "text-foreground"
                          )}>
                            {val}
                          </span>
                        )}
          ...
          <div className="relative overflow-hidden rounded-2xl bg-card border border-border shadow-lg">
            ...
            <div className="p-8">
              <div className="flex items-start gap-4 mb-7">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 bg-primary/10 border border-primary/10">
                ...
                <div>
                  ...
                  <p className="text-[13px] mt-1 leading-relaxed text-muted-foreground/60">
              ...
              <div className="mb-7">
                ...
                <p className="text-[12px] mt-0.5 text-muted-foreground/40">por mês</p>
              </div>
              ...
              <p className="text-[11px] mb-6 text-muted-foreground/35">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-3.5 font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-150 rounded-xl bg-accent text-foreground border border-border hover:bg-accent/80 hover:border-border/80"
              >
                Ativar notificações
        ...
        {/* Trust */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-10 text-xs text-muted-foreground/30">
