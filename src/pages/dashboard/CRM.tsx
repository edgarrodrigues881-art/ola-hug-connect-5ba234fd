...
import {
  Plus, Search, Pencil, Trash2, Phone, Mail, Building2, ChevronLeft, ChevronRight,
...
const CRM = () => {
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ nome: "", telefone: "", email: "", empresa: "", etapa: "Novo Lead", valor: "", observacao: "" });
    setDialogOpen(true);
  };

  const openEdit = (l: Lead) => {
    setEditingId(l.id);
    setForm({ nome: l.nome, telefone: l.telefone, email: l.email, empresa: l.empresa, etapa: l.etapa, valor: l.valor, observacao: l.observacao });
    setDialogOpen(true);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button size="sm" className="gap-1.5 text-xs bg-primary hover:bg-primary/90" onClick={openCreate}>
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="text-center text-sm text-muted-foreground py-8">Nenhum lead encontrado</TableCell>
              </TableRow>
            ) : filtered.map((l, idx) => (
              <TableRow key={l.id}>
                ...
                <TableCell className="text-xs text-muted-foreground">{l.telefone}</TableCell>
                <TableCell className="text-xs text-muted-foreground">{l.empresa}</TableCell>
                <TableCell>
                  <Badge variant="outline" className={`text-[10px] ${etapaColors[l.etapa] || ""}`}>{l.etapa}</Badge>
                </TableCell>
                <TableCell className="text-xs font-medium">{l.valor}</TableCell>
                <TableCell className="text-xs text-muted-foreground">{l.criadoEm}</TableCell>
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" className="text-xs h-7 gap-1" onClick={() => openEdit(l)}>
                      <Pencil className="w-3 h-3" /> Editar
                    </Button>
                    <Button variant="outline" size="sm" className="text-xs h-7 gap-1 text-destructive border-destructive/30 hover:bg-destructive/10" onClick={() => handleDelete(l.id)}>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input value={form.nome} onChange={e => setForm(p => ({ ...p, nome: e.target.value }))} placeholder="Nome completo" className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Telefone *</Label>
                <Input value={form.telefone} onChange={e => setForm(p => ({ ...p, telefone: e.target.value }))} placeholder="+55 11 99999-0000" className="h-9 text-sm" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs">Email</Label>
                <Input value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="email@exemplo.com" className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Empresa</Label>
                <Input value={form.empresa} onChange={e => setForm(p => ({ ...p, empresa: e.target.value }))} placeholder="Nome da empresa" className="h-9 text-sm" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.valor} onChange={e => setForm(p => ({ ...p, valor: e.target.value }))} placeholder="R$ 0,00" className="h-9 text-sm" />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Observação</Label>
              <Textarea value={form.observacao} onChange={e => setForm(p => ({ ...p, observacao: e.target.value }))} placeholder="Notas sobre o lead..." rows={3} className="text-sm" />
            </div>
          </div>
          <DialogFooter>
